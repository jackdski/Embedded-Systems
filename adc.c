...
//Import all of the variables from the main
...
//This function configures all of the ADC channels as well as the 1.0 LED and Port one button interrupts
void configure_ADC() {
    ...
    // Map temp analog channel to MEM0/MCTL0, set 1.2v ref
    ADC14->MCTL[0] = (ADC14_MCTLN_INCH_22 | ADC14_MCTLN_VRSEL_1 | ADC14_MCTLN_EOS);
    ...
    ADC14->IER0 |= ADC14_IER0_IE3 | ADC14_IER0_IE4 | ADC14_IER0_IE5; //Enable interrupts
...
//This function configures the UART pins to have a 9600 BAUD Rate
...
//This function sends an array of size length over uart
...
//This function sends one byte
void UART_send_byte(uint8_t data){
    EUSCI_A0->TXBUF = data;
}

//This is the UART handler
//It sends information and turns on the ADC when transmission is done
...
//This is the ADC handler. It pulls and stores all ADC information
...
//This is the port one handler. It either runs the printtemps function
//or sets up the problem6 function
...
//This function prints out the temp in K,C and F
void printTemps(){
    float Ctemp = 0;
    uint8_t CString[7];
...
//This function prints all 60 temperatures currently stored in the TXBuffer
//It also prints the location of the joystick
void problemSix() {
    while(!isEmpty(TXBuf)){
        float Ftemp = (0.0381)*(float)(removeItem(TXBuf))-360.5;
        uint8_t FString[6];
        ftoa((Ftemp),2,5,FString);
        FString[5] = 0x0D;
